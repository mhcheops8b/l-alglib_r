    ...
    for line in file.lines() {
        ...
        if cur_line_no == pord_num {
            ...
            for line in task_file.lines() {    
                ...
                let init_vector = line_str.split(",").map(|v| v.trim().parse().unwrap()).collect::<Vec<_>>();
                eprintln!("Init vector (int): {:?}", init_vector);
    
                l_alglib::lib_flatten::l_alg_gen_from_ord_short_time_with_limit_old(Duration::new(time_limit, 0), &flatten_pord, n, &init_vector, &mut lalgs, true, true);
