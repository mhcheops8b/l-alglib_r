...
fn main() {
    ...
    if args_len < 6 {
        println!("Usage: {} <limit in seconds> <pord_filename> <pord_num> <need transform> <task_file>", std::env::args().next().unwrap());
        return;
    }
    ...
    let b_need_transform = match std::env::args().nth(4).unwrap().parse::<usize>() {
            Ok(val) => {match val {
                                0=> false, 
                                1=>true, 
    ...
    for line in file.lines() {
        ...
        if cur_line_no == pord_num {
            ...
            let mut lalgs = HashSet::<Vec<usize>>::new();
            ...
            for line in task_file.lines() {    
                ...
                let mut init_vector = line_str.split(",").map(|v| v.trim().parse().unwrap()).collect::<Vec<_>>();
                ...
                l_alglib::lib_flatten::l_alg_gen_from_ord_short_time_with_limit_new(Duration::new(time_limit, 0), &flatten_pord, n, &init_vector, &mut lalgs, true, true);
